...
export function NewCharExpUpgrade(k: KAPLAYCtx) : void
{
    
    k.scene("newCharExpUpgrade",  () => {
        AppStore.SetState((prevState) => ({
            ...prevState,
            currentScene: "newCharExpUpgrade",
            previousScene: prevState.currentScene,
        }), "CreateNewCharExperiencesScene");
        ...
        const subtitleText = fullBorder.add([
            k.text("Add 1 Experience to Your Character or click on an Experience to increase its level.", {size: 20, font: "monogram", width: k.width() - 40, align: "center"}),
        ...
        function AddExperienceToList()
        {
            ...
            if (experienceValue !== "")
            {
                AppStore.GetState().player.experienceList.push({ name: experienceValue, level: 1 });
        ...
        const finishButtonText = finishButton.add([
            k.text("Finish", {size: 24, font: "monogram", width: finishButton.width - 20, align: "center"}),
            k.pos(0, 0),
            k.anchor("center"),
            k.color(k.rgb(5, 5, 5)),
        ]);
        ...
        finishButton.onMouseRelease(() => {
            if (isFinishButtonHovered)
            {
                ...
                k.wait(0.1, () => {
                    Firebase.Publish(`players/${AppStore.GetState().player.characterID}/experienceList`, AppStore.GetState().player.experienceList);
                    k.go("charSheet");
                });
            }
        ...
        function Render()
        {
            ...
            for (const exp of AppStore.GetState().player.experienceList)
            {
